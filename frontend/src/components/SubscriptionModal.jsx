...
function validateForm(form) {
  ...
  if (!form.renewalDate) {
    nextErrors.renewalDate = "Renewal date is required.";
  }
        ...
        <div className="flex flex-1 flex-col gap-5 overflow-y-auto bg-white p-6">
          ...
          <div className="flex gap-4">
            ...
            <Field label="Price" error={fieldErrors.price}>
              <div className="relative">
                ...
                <input aria-label="Price" aria-invalid={Boolean(fieldErrors.price)} type="number" step="0.01" value={form.price} onChange={(event) => change("price", event.target.value)} placeholder="0.00" className="w-full rounded-xl border border-slate-200 bg-slate-50 py-3 pl-7 pr-3 text-[14px] font-medium text-slate-900 outline-none transition-all focus:border-slate-400 focus:bg-white focus:ring-4 focus:ring-slate-100" />
              </div>
            </Field>
          </div>
          ...
          <div className="grid gap-4 sm:grid-cols-2">
            <Field label="Category">
              <select aria-label="Category" value={form.categoryId} onChange={(event) => change("categoryId", event.target.value)} className="w-full appearance-none rounded-xl border border-slate-200 bg-slate-50 px-3.5 py-3 text-[14px] font-medium text-slate-900 outline-none transition-all focus:border-slate-400 focus:bg-white focus:ring-4 focus:ring-slate-100">
                <option value="">Category</option>
                {categories.map((category) => <option key={category.id} value={category.id}>{category.name}</option>)}
              </select>
            </Field>
            <Field label="Billing Cycle" error={fieldErrors.billingCycle}>
              <select aria-label="Billing Cycle" aria-invalid={Boolean(fieldErrors.billingCycle)} value={form.billingCycle} onChange={(event) => change("billingCycle", event.target.value)} className="w-full appearance-none rounded-xl border border-slate-200 bg-slate-50 px-3.5 py-3 text-[14px] font-medium text-slate-900 outline-none transition-all focus:border-slate-400 focus:bg-white focus:ring-4 focus:ring-slate-100">
                {Object.entries(cycleLabels).map(([value, key]) => <option key={value} value={value}>{t[key]}</option>)}
              </select>
            </Field>
            <Field label="Renewal Date" error={fieldErrors.renewalDate}>
              <input aria-label="Renewal Date" aria-invalid={Boolean(fieldErrors.renewalDate)} type="date" value={form.renewalDate} onChange={(event) => change("renewalDate", event.target.value)} className="w-full rounded-xl border border-slate-200 bg-slate-50 px-3.5 py-3 text-[14px] font-medium text-slate-900 outline-none transition-all focus:border-slate-400 focus:bg-white focus:ring-4 focus:ring-slate-100" />
            </Field>
          ...
          <div className="mt-1 flex items-center justify-between rounded-xl border border-slate-200 bg-slate-50 p-3.5">
            ...
            <button type="button" aria-label="Toggle active status" onClick={toggleActive} className={`relative flex h-7 w-12 items-center rounded-full pl-0.5 shadow-inner transition-colors ${form.status === "ACTIVE" ? "bg-[#7B42FF]" : "bg-slate-300"}`}>
          ...
          <Field label="Description (Optional)">
            <textarea aria-label="Description" rows={2} value={form.description} onChange={(event) => change("description", event.target.value)} placeholder="Add notes, e.g. shared with family..." className="w-full resize-none rounded-xl border border-slate-200 bg-slate-50 px-3.5 py-3 text-[14px] font-medium text-slate-900 outline-none transition-all focus:border-slate-400 focus:bg-white focus:ring-4 focus:ring-slate-100" />
          </Field>
        </div>
